...
{
    //Tworzymy nowy obiekt klasy kopca i wchodzimy do pętli obsługi opcji.
    heap = new BinaryHeap();
    ...
    //Po wyjściu z pętli dealokujemy kopiec.
    delete heap;
}
...
{
    //Dealokujemy kopiec i alokujemy pamięć na nowy obiekt.
    delete heap;
    heap = new BinaryHeap();
    int size;
...
{
    //Wyświetlamy wizualizację drzewa.
    std::cout << std::endl;
    heap->printTree();
}
